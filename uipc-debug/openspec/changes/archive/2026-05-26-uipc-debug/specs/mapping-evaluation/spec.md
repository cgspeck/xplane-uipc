...
### Requirement: Variable breakdown toggle
The tool SHALL support switching between display modes for expression mappings in the table: showing the expression string inline, or showing the per-variable input values inline. This SHALL be toggled per-row via the detail popup.

#### Scenario: Toggle display mode from popup
- **WHEN** the expression detail popup is open
- **THEN** the user can toggle the table display mode for that row between expression string and per-variable values
