...
- [ ] 1.1 In `iterate_records` in `ipc_host/src/mapped_view.rs`: change the
...
- [ ] 1.2 Remove the `LOGGED_SENTINEL_VALUES` HashSet, the `FSD_LOGGED_TEXTS`
       HashSet, and the `LazyLock`/`Mutex` wrappers.
- [ ] 1.3 Remove `reset_logged_sentinels()` function (callers too).
- [ ] 1.4 Downgrade the per-record `tracing::debug!` at line 173
       ("Bad sentinel: reqID=...") to `tracing::trace!`.
- [ ] 1.5 The recovery-scanning code (Phase 1 / Phase 2) remains for the
...
- [ ] 2.1 In `ipc_host/examples/capture-inspect.rs`: widen the output to
       handle `sentinel_ok = false` records — show them as valid records
       with a marker (e.g., trailing `?` or different indicator) instead
       of `"BAD SENTINEL"`.
- [ ] 2.2 Remove `any_corrupted` tracking that only fired on sentinel
...
- [ ] 3.1 Copy capture files to `ipc_host/tests/fixtures/` with stable names:
...
- [ ] 3.2 Create `ipc_host/tests/capture_records.rs`:
...
- [ ] 4.1 Run `cargo test -p ipc_host` to confirm all tests pass
- [ ] 4.2 Run `cargo build` to confirm compilation
- [ ] 4.3 Run `cargo fmt` to format code
